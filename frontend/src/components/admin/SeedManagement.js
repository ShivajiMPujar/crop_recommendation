...
const SeedManagement = () => {
  ...
  const handleAddSeed = async (e) => {
    ...
    try {
      ...
      if (res.status === 201 || res.status === 200) {
        ...
        alert(editingSeed ? t('seedUpdateSuccess') : t('seedAddedSuccess'));
      }
    } catch (err) {
      console.error('Error adding/updating seed:', err);
      alert(t('errorSavingSeed'));
    }
    setLoading(false);
  };
  ...
  return (
    <div>
      <div className="mb-8">
        <h2 className="text-3xl font-bold text-gray-900 mb-2">🌾 {t('seedManagement')}</h2>
        <p className="text-gray-600">{t('manageSeedDesc')}</p>
      </div>

      {/* Existing Seeds Grid Display */}
      {seeds.length > 0 && (
        <div className="mb-12">
          <h3 className="text-2xl font-bold text-gray-900 mb-6">{t('existingSeeds')}</h3>
                ...
                <div className="p-6 space-y-3">
                  {seed.cropName && (
                    <div className="flex justify-between items-center">
                      <span className="font-medium text-gray-600 text-sm">🌱 {t('crop')}:</span>
                  ...
                  {seed.type && (
                    <div className="flex justify-between items-center">
                      <span className="font-medium text-gray-600 text-sm">🏷️ {t('type')}:</span>
                  ...
                  {seed.pricePerKg && (
                    <div className="flex justify-between items-center">
                      <span className="font-medium text-gray-600 text-sm">💰 {t('price')}:</span>
                  ...
                  {seed.germinationRate && (
                    <div className="flex justify-between items-center">
                      <span className="font-medium text-gray-600 text-sm">🌱 {t('germination')}:</span>
                  ...
                  {seed.duration && (
                    <div className="flex justify-between items-center">
                      <span className="font-medium text-gray-600 text-sm">📅 {t('duration')}:</span>
                      <span className="text-gray-800 text-sm">{seed.duration} {t('days')}</span>
                    </div>
                  )}

                  {seed.rating !== undefined && (
                    <div className="flex justify-between items-center">
                      <span className="font-medium text-gray-600 text-sm">⭐ {t('rating')}:</span>
      ...
      <div className="bg-pista-50 p-6 rounded-xl border-2 border-gray-200 mb-8">
        <h3 className="text-2xl font-bold text-gray-900 mb-6">{editingSeed ? t('updateSeed') : t('addSeedBrand')}</h3>
        <form onSubmit={handleAddSeed} className="space-y-6">
          {/* Row 1: Name, Crop */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">{t('seedNameLabel')} *</label>
              <input
                value={newSeed.name}
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">{t('cropNameLabel')} *</label>
              <input
                value={newSeed.crop}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">{t('pricePerKg')}</label>
              <input
                type="number"
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">{t('germinationRatePercent')}</label>
              <input
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">{t('duration')} ({t('days')})</label>
              <input
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">{t('ratingStars')}</label>
          ...
          <div className="flex gap-4">
            <button
              ...
              disabled={loading}
            >
              {loading ? t('saving') : (editingSeed ? t('updateSeed') : t('addSeedBrand'))}
            </button>
            {editingSeed && (
              <button
                type="button"
